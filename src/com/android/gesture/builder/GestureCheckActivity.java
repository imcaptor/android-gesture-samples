...
public class GestureCheckActivity extends Activity implements OnGesturePerformedListener {
    ...
    public void onGesturePerformed(GestureOverlayView overlay, Gesture gesture) {
        ...
        if (predictions.size() > 0) {
            for (Prediction prediction : predictions) {
                ArrayList<Gesture> gestures = mStore.getGestures(prediction.name);
                // 笔画相同是前提。
                if (gestures.size() > 0
                        && gesture.getStrokesCount() == gestures.get(0).getStrokesCount()) {
    ...
    /***
     * @Override public boolean onCreateOptionsMenu(Menu menu) { boolean rt =
     *           super.onCreateOptionsMenu(menu);
     *           menu.add(R.string.gestures_list); return rt; } public boolean
     *           onOptionsItemSelected(MenuItem item) { switch
     *           (item.getItemId()) { case MENU_ID_LIST: return true; } return
     ...
     * GestureOverlayView.OnGestureListener { Gesture mGesture; public void
     * onGestureStarted(GestureOverlayView overlay, MotionEvent event) {
     * Log.d(TAG, "onGestureStarted"); mGesture = null; } public void
     * onGesture(GestureOverlayView overlay, MotionEvent event) { Log.d(TAG,
     * "onGesture"); } public void onGestureEnded(GestureOverlayView overlay,
     * MotionEvent event) { mGesture = overlay.getGesture(); Log.d(TAG,
     * "onGestureEnded:" + mGesture); if (mGesture.getLength() <
     * LENGTH_THRESHOLD) { overlay.clear(false); } } public void
